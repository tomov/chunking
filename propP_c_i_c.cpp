// compile with:
//
// mex propP_c_i_c.cpp printmex.cpp -I/usr/local/boost-1.64.0/include/
...
public:
  ...
  operator()(ArgumentList outputs, ArgumentList inputs) { 
    ...
    // check i 
    const TypedArray<double> _i = inputs[0];
    int i = _i[0];
    DEBUG_PRINT("i = %d\n", i);

    // check D TODO dedupe with sample_c.cpp
    StructArray const matlabStructArrayD = inputs[2];
    ...
    // check h
    StructArray const matlabStructArrayHyperparams = inputs[3];
    ...
    // return P 
    //
    Array resultP = factory.createArray<std::vector<double>::iterator, double>({1, P.size()}, P.begin(), P.end());
