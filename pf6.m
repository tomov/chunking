% this is the same as particle folder toexcept with adding the log likelihood online
...
filename = sprintf('mat/pf6_alpha=%.4f_nsamples=%d_div_eps=%.4f_last_np=%d.mat', h.alpha, nsamples, h.eps, num_particles);
...
for subj = 1:length(D) % for each subject
    ...
    for t = 1:T
        ...
        for i = 1:num_particles
            ...
            log_w(i) = log_w(i) + pf_loglik(t, particles(i), D(subj), h);
        end
